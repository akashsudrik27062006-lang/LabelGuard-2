 ...
 function VersionComparison() {
  ...
  React.useEffect(() => {
    ...
    setLoading(true);
    compareVersionsDetailed(verAId, verBId).then(setCompareData).finally(() => setLoading(false));
  }, [verAId, verBId]);
            ...
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '20px' }}>
              <div>
                ...
                <div style={{ marginTop: '10px' }}>
                  {a.imageUrl ? <img src={a.imageUrl} style={{ width: '100%', maxWidth: '260px', border: '1px solid var(--border-light)', borderRadius: 'var(--radius-sm)' }} /> : <p style={{ fontSize: '13px', color: 'var(--text-muted)' }}>No image</p>}
                </div>
              </div>
              <div>
                <span className="eyebrow">{bLabel} — REVISED ARTWORK</span>
                <div style={{ marginTop: '10px' }}>
                  {b.imageUrl ? <img src={b.imageUrl} style={{ width: '100%', maxWidth: '260px', border: '1px solid var(--border-light)', borderRadius: 'var(--radius-sm)' }} /> : <p style={{ fontSize: '13px', color: 'var(--text-muted)' }}>No image</p>}
                </div>
              </div>
            </div>
          </section>
