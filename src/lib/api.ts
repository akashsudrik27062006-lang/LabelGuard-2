  ...
  export async function nextVersionLabel(productGroup: string): Promise<string> {
    ...
    if (error) throw error;
    const nums = (data ?? []).map(p => parseFloat((p.version_label ?? 'v1.0').replace(/^v/i, ''))).filter(n => !isNaN(n));
    if (nums.length === 0) return 'v1.0';
    return 'v' + (Math.round((Math.max(...nums) + 0.1) * 10) / 10).toFixed(1);
  }
  ...
  export async function listProductGroups() {
    ...
    return Array.from(groups.values()).map(g => {
      ...
      return {
        ...
        currentVersionLabel: latestVersion.version_label ?? 'v1.0',
  ...
  export async function listVersionsForGroup(productGroup: string) {
    ...
    return versionProducts.map(p => {
      ...
      return {
        productId: p.id,
        versionLabel: p.version_label ?? 'v1.0',
        scanId: scan?.id ?? null,
    ...
    return (violations ?? [])
      ...
      .map((v: any) => {
        ...
        return {
          ...
          versionLabel: product?.version_label ?? 'v1.0',
